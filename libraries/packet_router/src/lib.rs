...
impl<T: PacketTrait> Router<T> {
    ...
    pub fn poll(&mut self) {
        ...
        let clients_by_address: HashMap<u16, Rc<RefCell<Client<T>>>> = self.clients_by_address.iter()
            .filter_map(|(address, client_weak)| {
                match client_weak.upgrade() {
                    Some(c) => Some((*address, c)),
                    None => None,
                }
            })
            .collect(); 
        ...
        for (address, client) in clients_by_address.iter() {
            ...
            for topic in subscriptions.iter() {
                address_by_topic.entry(topic.clone()).or_insert(Vec::new()).push(*address);
            }
        }
