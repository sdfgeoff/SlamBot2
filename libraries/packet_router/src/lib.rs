    ...
    pub fn poll(&mut self) {
        ...
        for packet in all_outgoing_packets.iter() {
            if let Some(to_address) = packet.get_to() {
                ...
                if topic == "all" {
                    for address in clients_by_address.keys() {
                        packets_for_addresses
                            .entry(*address)
                            .or_default()
                            .push(packet.clone());
                    }
                } else if let Some(topic_addresses) = address_by_topic.get(topic) {
...
mod tests {
    ...
    fn test_multiple_subscriptions() {
        ...
        client1.borrow_mut().send_packet(TestPacket::new("topic1".to_string(), "msg1".to_string()));
        client1.borrow_mut().send_packet(TestPacket::new("topic2".to_string(), "msg2".to_string()));
    ...
    fn test_multiple_packets_same_poll() {
        ...
        client1.borrow_mut().send_packet(TestPacket::new("test".to_string(), "msg1".to_string()));
        client1.borrow_mut().send_packet(TestPacket::new("test".to_string(), "msg2".to_string()));
        client1.borrow_mut().send_packet(TestPacket::new("test".to_string(), "msg3".to_string()));
